...
public class T extends Applet implements Runnable {
	...
	public void run() {
		...
		final String TERRAIN_ELEVATIONS = 
				"u0055u0056u0058u005Du0065u007F";
		...
		final String TERRAIN_PALETTE = 
				"u0066u007Fu0072u0033u0049" +				// water
				"u0049u0059u007Fu0046u0039" +				// sand
				"u0053u006Cu0066u004Fu0026" +				// grass
				"u0053u007Cu004Cu002Cu0039" +				// forest
				"u0040u0040u0042u0040u0072" +				// mountain slope
				"u0040u0040u0042u0040u0072";				// mountain peak
		...
		for (i = 0; i < LAND_TYPES; i++) {
			...
			for (pi = px; pi < py; pi++) {
				...
				if (i == 0) {			// Water: Cubic trigonometric interpolation
					...
					d = -Math.cos(WAVE_FREQUENCY * t*t*t * Math.PI);
		...
		for (i = 0; i < GRAD_NUM; i++) {
			...
			d = 2*Math.PI * (double)(2*i + 1) / (double)(GRAD_NUM*2);
		...
		while (true) {
			...
			for (k = 0; k < RESOURCE_NUM_TYPES; k++) {
				resourceTypeID[k] = (int)(Math.random() * pSeed);
			}			
			...
			while (gameState != STATE_GAMEOVER || keys[KEY_S] == 0) {
				...
				if (gameState != STATE_GAMEOVER) {
					...
					if (keys[MOUSE_LMB] > keys[INPUT_PREVIOUS + MOUSE_LMB] || keys[MOUSE_DRAG] > 0) { 
						...
						playerTargetRot = Math.atan2((double)(keys[MOUSE_Y] - WORLD_SIZE/2), (double)(keys[MOUSE_X] - WORLD_SIZE/2)); // + (2*Math.PI)); // % (2*Math.PI); 
					}
					...
					if (!playerAnchorDropped) {
						...
						playerVel[0] += WIND_SPEED * Math.cos(playerRot);
						playerVel[1] += WIND_SPEED * Math.sin(playerRot);
					}
						...
						for (py = qy-k; py < qy+k+1; py++) {
							for (px = qx-k; px < qx+k+1; px++) {
								...
								if (world[(py & (WORLD_SIZE-1))*WORLD_SIZE + (px & (WORLD_SIZE-1))] >= t) {
									...
									ux = tx*Math.cos(playerRot) + ty*Math.sin(playerRot);
									uy = ty*Math.cos(playerRot) - tx*Math.sin(playerRot);
